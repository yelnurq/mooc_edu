...
const AIChat = () => {
  ...
  const [chatHistory, setChatHistory] = useState([
    { 
      role: 'assistant', 
      content: "Привет! Я твой академический помощник КазУТБ им. К. Кулажанова. Выбери дисциплину или просто задай вопрос по обучению, тестам или сертификатам." 
    }
  ]);
  
  const scrollRef = useRef(null);

  // 1. Загрузка активных курсов
  useEffect(() => {
  ...
  // 2. Исправленная логика автопрокрутки
  useEffect(() => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setError("Ошибка соединения с нейросетью");
      setChatHistory(prev => [...prev, { 
        role: 'assistant', 
  ...
  return (
    <main className={`transition-all duration-500 flex flex-col font-sans ${
      isFullScreen 
      ? 'fixed inset-0 z-[9999] bg-[#f8fafc] p-0' 
      : 'max-w-[1400px] mx-auto px-6 py-8 bg-[#f8fafc] min-h-screen'
    }`}>
      
      {/* HEADER */}
      <div className={`flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-slate-200 pb-8 mb-8 ${isFullScreen ? 'px-8 pt-8' : ''}`}>
      ...
      <div className={`grid grid-cols-1 lg:grid-cols-4 gap-8 flex-1 overflow-hidden ${isFullScreen ? 'px-8 pb-8' : ''}`}>
        ...
        <div className="lg:col-span-1 space-y-4 overflow-y-auto pr-2">
          ...
          <div className="p-6 bg-blue-600 rounded-2xl text-left text-white shadow-lg relative overflow-hidden group">
             <div className="relative z-10">
                <p className="text-[10px] font-black uppercase tracking-widest opacity-80 mb-2">Инфо</p>
                <p className="text-xs font-bold leading-relaxed">
                  Проходите модули и тесты, чтобы получить доступ к экзамену и сертификату.
                </p>
             </div>
             <GraduationCap size={100} className="absolute -right-4 -bottom-4 opacity-10 transform rotate-12" />
          </div>
        </div>

        {/* MAIN CHAT */}
        <div className="lg:col-span-3 flex flex-col bg-white rounded-3xl border border-slate-200  overflow-hidden relative">
          
          {/* Messages Area */}
          <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-8 bg-[#fcfdfe] scroll-smooth">
              ...
              {chatHistory.map((msg, idx) => (
                ...
                >
                  ...
                  <div className={`flex flex-col max-w-[80%] ${msg.role === 'user' ? 'items-end text-right' : 'items-start text-left'}`}>
                    ...
                    <div className={`p-5 rounded-2xl text-[13px] font-medium leading-relaxed shadow-sm ${
                      msg.role === 'user' 
          ...
          <div className="p-6 md:p-8 bg-white border-t border-slate-100">
            {selectedCourse && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-2 mb-4 bg-slate-900 w-fit px-3 py-1.5 rounded-lg shadow-lg">
            ...
            <div className="relative">
              <textarea
                ...
                className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-5 pr-20 text-[14px] font-medium text-slate-900 focus:border-blue-600 focus:bg-white outline-none transition-all resize-none min-h-[100px] shadow-inner"
              />
            ...
            <div className="mt-6 flex justify-between items-center opacity-50">
               <div className="flex gap-4">
                 <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                   <ShieldCheck size={12} className="text-emerald-500" /> Secure Link
                 </p>
                 <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                   <MessageSquare size={12} /> Academic GPT
                 </p>
               </div>
               
               <button 
                 onClick={() => { setChatHistory([]); setSelectedCourse(null); }}
                 className="text-[9px] font-black text-slate-400 uppercase tracking-widest hover:text-red-500 transition-all flex items-center gap-1.5"
               >
                 <RotateCcw size={12} /> Сбросить чат
               </button>
            </div>
          </div>
        </div>
