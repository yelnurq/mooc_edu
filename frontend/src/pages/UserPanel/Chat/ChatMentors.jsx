...
import { 
  ...
  Phone, Star, CheckCheck, Loader2, Smile, Sparkles, BookOpen
} from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import api from '../../../api/axios';

// Используем тот же компонент сообщения, что и в AIChat
...
const CourseMentorsChat = () => {
  ...
  return (
    <div className="flex h-screen bg-slate-100 font-sans overflow-hidden">
      ...
      {/* 1. SIDEBAR (Стилизация под AIChat) */}
            ...
            >
              ...
              <div className="flex-1 text-left overflow-hidden">
                <div className="flex justify-between items-center mb-0.5">
                  <span className="text-[14px] font-bold text-slate-900 truncate">{room.author.name}</span>
      ...
      <div className="flex-1 flex flex-col bg-white relative">
        {selectedRoom ? (
          <>
            ...
            <div className="h-20 px-8 border-b border-slate-100 flex items-center justify-between bg-white/95 backdrop-blur-md z-10 shadow-sm">
              ...
              <div className="flex items-center gap-3">
                 ...
                 <button className="p-3 text-slate-400 hover:text-yellow-500 hover:bg-yellow-50 rounded-xl transition-all"><Star size={20}/></button>
              </div>
            </div>

            {/* Messages Area with Fixed Pattern */}
            ...
            {/* Input Area (Стилизация под AIChat) */}
            <div className="p-6 bg-white border-t border-slate-100 z-10">
              <div className="max-w-4xl mx-auto flex items-center gap-4 bg-slate-50 p-2 rounded-[28px] border border-slate-200 shadow-inner">
          ...
          <div className="flex flex-col items-center justify-center h-full bg-slate-50 text-slate-300">
            ...
            <p className="text-[12px] font-black uppercase tracking-[0.2em] text-slate-400">Выберите диалог для общения</p>
          </div>
        )}
      </div>
